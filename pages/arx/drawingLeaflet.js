...
*/
	...
	var drawSVG = function(attr)
	{
		...
		if(layerItems.length == 0) {
			...
			layerItems[2]._container.style.pointerEvents = 'painted';
			...
			//layerItems[1]._container.style.pointerEvents = 'visiblePainted';
/*
			...
			var downArr = [];
			layerItems[1].on('click', function(e) {
				downArr.push(new Date().getTime());
				if(downArr.length > 1) {	// имитация dblclick
					var delta = downArr[1] - downArr[0];
					if(downArr.length > 2) downArr.shift();
...
*/
	...
	var editObject = function(coords, props, editType, propHiden)
	{
		...
		{
			//if(initMove) initMove(ph);
			if(ph.attr) ph = ph.attr;
			var latlng = ph.latlng;
			var x = latlng.lng;
			if(x < -180) x += 360;
//console.log('mouseMove:  ', editIndex, skipClick);
			if(editIndex != -1) {
...
console.log('dblclick1:  ', coords.length);
					...
					if(editType === 'POLYGON') {
						...
						//layerItems[2].options['skipLastPoint'] = false;
/*						if(downType.num === 0 || downType.num === len) {
						//if(downType.num == 0 && len > 0) {
							...
							return;
							len++;
							//coords[len][0] = coords[0][0];
							//coords[len][1] = coords[0][1];
						}*/
						if(len == 2) len = 0;
					} else if(editType === 'LINESTRING') {
	console.log('dblclick1:  ', coords.length);
						layerItems[1]._container.style.pointerEvents = 'visiblePainted';	// после onFinish без drag карты
						layerItems[2]._container.style.pointerEvents = 'visiblePainted';	// после onFinish без drag карты
						layerItems[2].options['skipLastPoint'] = false;
						ret.stopDrawing();
...
console.log('onFinish:  ', coords.length);
	...
	console.log('onFinish1:  ', coords.length);
						layerItems[1]._container.style.pointerEvents = 'visiblePainted';	// после onFinish без drag карты
						layerItems[2]._container.style.pointerEvents = 'visiblePainted';
						layerItems[2].options['skipLastPoint'] = false;
						ret.stopDrawing();
...
console.log('click:  ', len, downType.num, isFinish);
						...
						if(editType === 'LINESTRING') {
							...
							if(editIndex === -1) {
								...
								layerItems[1]._container.style.pointerEvents = 'none';	// после onFinish без drag карты
								layerItems[2]._container.style.pointerEvents = 'none';
								layerItems[2].options['skipLastPoint'] = false;
					gmxAPI._drawing['activeState'] = true;
							drawAttr['coords'] = coords;
							drawSVG(drawAttr);
							}
console.log('click1:  ', len, downType.num, isFinish);
							return;
						}
						/*} else if(editType === 'LINESTRING') {
							if(downType.type === 'node' && downType && (downType.num === 0 || downType.num == coords.length - 1)) {
								var len = coords.length - 1;
								var x = e.latlng.lng;
								if(x < -180) x += 360;
								var y = e.latlng.lat;
								if(downType.num === 0) coords = coords.reverse();
								editIndex = -1;
								//lastPoint = {'x': x, 'y':y};
								layerItems[2].options['skipLastPoint'] = true;
								layerItems[1]._container.style.pointerEvents = 'visiblePainted';
...
console.log('layerGroup click:  ', lastPoint, downType);
					var eObj = (domObj.propHiden['multiObj'] ? domObj.propHiden['multiObj'] : domObj);
...
/*			
			...
			{
console.log('item mousedown:  ', lastPoint, mousePressed);
			...
			{
console.log('dblclick:  ', coords.length);
...
/*		
		...
		var setEditItem = function(ph)
		{
			...
			if('type' in downType) {
				...
				mousePressed = true;
//console.log('setEditItem:  ', lastPoint, mousePressed);
				repaint();
				gmxAPI._cmdProxy('startDrawing');
				gmxAPI._drawing['activeState'] = true;
				if(!onMouseMoveID) onMouseMoveID = gmxAPI.map.addListener('onMouseMove', mouseMove);
		...
		{
//console.log('itemMouseDown:  ', lastPoint, mousePressed);
			if(mousePressed) {
				mouseUp();
				return;
			}
