...
},{"dup":1,"underscore":5}],3:[function(require,module,exports){
!function() {
  var d3 = {
    version: "3.5.5"
  };
...
},{"_process":32}],20:[function(require,module,exports){
/** @license MIT License (c) copyright 2010-2014 original author or authors */
/** @author Brian Cavalier */
/** @author John Hann */
 ...
 */
module.exports = function earth(bridge) {
    "use strict";
    ...
    var OVERLAY_ALPHA = 112;                  // overlay transparency (on scale [0, 255])
    var BRIGHT_OVERLAY_ALPHA = 160;           // overlay transparency (on scale [0, 255])
    ...
    var buttonStateNotifier = newAgent();  // notifies consumers of button state changes
    ...
    function locate(lon, lat) {
        ...
        var coord = [lon, lat], rotate = globe.locate(coord);
        if (rotate) {
            globe.projection.rotate(rotate);
            configuration.save({orientation: globe.orientation()});  // triggers reorientation
        }
        inputController.trigger("click", globe.projection(coord), coord);
    }
    ...
    function buildRenderer(mesh, globe) {
        ...
        function drawLocationMark(point, coord) {
            ...
            if (coord && _.isFinite(coord[0]) && _.isFinite(coord[1])) {
                ...
                if (!mark.node()) {
                    mark = foregroundSvg.append("path").attr("class", "location-mark");
                }
    ...
    function createMask(globe) {
        ...
        context.fillStyle = context.strokeStyle = "rgba(255, 0, 0, 1)";
        context.fill();
        //context.lineWidth = 2;
        //context.stroke();  // removes mask red on left
    ...
    function interpolateField(globe, grids) {
        ...
        if (!interpolate || !overlayInterpolate) return null;
                    ...
                    if (coord) {
                        ...
                        if (isFinite(λ)) {
                            ...
                            if (hasDistinctOverlay) {
                                ...
                                if (_.isArray(scalar)) scalar = scalar[2];
                            }
                            if (scalar === scalar) {
                                color = colorScale.gradient(scalar, OVERLAY_ALPHA);
                            }
                        }
                    }
                    mask.set(x, y, color).set(x+1, y, color).set(x, y+1, color).set(x+1, y+1, color);
                    wind = wind || HOLE_VECTOR;
                }
                row[i+3] = row[i+0] = wind[0];
                row[i+4] = row[i+1] = wind[1];
                row[i+5] = row[i+2] = wind[2];
            }
            rows[y+1] = rows[y] = row;
        }
    ...
    function brighten(data) {
        for (var i = 3; i < data.length; i += 4) {
            if (data[i] !== 0) {
                data[i] = BRIGHT_OVERLAY_ALPHA;
            }
        }
    }
    ...
    function drawOverlay(field, overlayType, animate) {
        ...
        if (overlayType) {
            if (overlayType !== "off") {
                if (!animate) {
                    // No animation, so make the overlay more opaque.
                    brighten(field.overlay.data);
                }
    ...
    function validityDate(grids) {
        if (grids) {
            return grids.overlayGrid.date.getTime();  // Use grid date if we have it.
        }
        ...
        return parts[0] === "current" ? null : Date.UTC(+parts[0], parts[1] - 1, +parts[2], +hhmm.substring(0, 2));
    }
    ...
    function showDate(grids) {
        ...
        var formatted = date ? (isLocal ? µ.toLocalISO(date) : µ.toUTCISO(date)) : "";
    ...
    function showLocationDetails(point, coord) {
        ...
        if (!field || point.length < 2 || !field.isInsideBoundary(point[0], point[1])) {
            return;
        }
        ...
        var formattedCoord = null;
        if (_.isFinite(λ) && _.isFinite(φ)) {
            d3.select("#location-coord").text(formattedCoord = µ.formatCoordinates(λ, φ));
            d3.select("#location-close").classed("invisible", false);
        }

        var values = {}, o = {};
        if (field.isDefined(point[0], point[1]) && grids) {
            var vector = grids.primaryGrid.interpolate(λ, φ);
            if (vector) {
                showWindAtLocation(vector, grids.primaryGrid);
                o = {};
                grids.primaryGrid.units.forEach(function(unit) {
                    o[unit.label] = µ.formatVector(vector, unit);
                });
                values[grids.primaryGrid.type] = o;
            }
            if (grids.overlayGrid !== grids.primaryGrid) {
                var scalar = grids.overlayGrid.interpolate(λ, φ);
                if (_.isArray(scalar)) {
                    scalar = scalar[2];  // use vector magnitude
                }
                if (scalar === scalar) {
                    showOverlayValueAtLocation(scalar, grids.overlayGrid);
                    o = {};
                    grids.overlayGrid.units.forEach(function(unit) {
                        o[unit.label] = µ.formatScalar(scalar, unit);
                    });
                    values[grids.overlayGrid.type] = o;
                }
            }
        }

        if (bridge.showLocationDetails && formattedCoord !== null) {
            bridge.showLocationDetails(formattedCoord, JSON.stringify(values));
        }
    }
...
//        });
        ...
        bindButtonToConfiguration("#animate-stop", {animate: false});
        bindButtonToConfiguration("#animate-start", {animate: true});
    ...
    _.extend(earth, {
        ...
        navToDate: function(date) {
            ...
            target = product.navigateTo(target);
            if (!target) return 0;
            _.defer(function() {
        ...
        click: function(id) {
            ...
            _.defer(function() {
                d3.select("#" + id).node().dispatchEvent(µ.newClickEvent());
            });
        },
...
},{"./globes":28,"./micro":30,"./products":31,"Backbone":1,"d3":3,"topojson":4,"underscore":5,"when":23}],28:[function(require,module,exports){
/**
...
},{"./micro":30,"d3":3,"underscore":5}],29:[function(require,module,exports){
(function (global){
!function() {
    "use strict";
    ...
    global.main = function(bridge) {
        _.defer(earth, bridge);  // defer all side effects
    };

    function disallowed(referrer) {
        var host = referrer.split("/")[2] || "";
        var blacklist = [
            /xtremeweatherforecast/i
        ];
        for (var i = 0; i < blacklist.length; i++) { if (blacklist[i].test(host)) return true; }
        return false;
    }

    if (µ.isEmbeddedInIFrame() && disallowed(document.referrer)) {
        return d3.select("body").attr("style", "font-size: 12em; color: red;").text("XX").selectAll("*").remove();
    } else if (_.has(µ.parseQueryString(window.location.search), "app")) {
        ...
        var stub = {
            ...
            time: function(msg)    { console.log("[TIME]", msg); },
            timeEnd: function(msg) { console.log("[END]", msg); },

            /**
             * The product has changed, having these new values. Note this is also a signal the scale is now dirty.
             *
             * @param date date and time in milliseconds (epoch)
             * @param description description of the data. Never null.
             * @param source who provided the data, string to be displayed after "Source |". Never null.
             */
            productChange: function(date, description, source) {
                console.log("productChange:", date, description, source);
            },

            /**
             * Indicates which buttons have changed states according to the provided lists of button ids.
             * All buttons not mentioned are assumed to have the default enabled state.
             *
             * @param highlighted JSON array of highlighted button ids: ["id1", "id2", ..., "idn"]. Never null.
             * @param disabled JSON array of disabled button ids: ["id1", "id2", ..., "idn"]. Never null.
             */
            buttonStateChange: function(highlighted, disabled) {
                console.log("buttonStateChange:", highlighted, disabled);
            },

            /**
             * Show status message to user. Replaces previous message, if any. Will be "" when message should be cleared.
             *
             * @param msg the message: "Loading...", "Rendering globe...", "No Data", etc. Never null.
             * @param isError true if the message is the result of an error
             */
            showMessage: function(msg, isError) {
                console.log("showMessage:", msg, isError);
            },

            /**
             * Show progress indicator to user. Amount increases each successive call. First call is always 0, last call
             * is always 100, with a variable number of intermediate calls.
             *
             * @param amount [0, 100] (inclusive)
             */
            showProgress: function(amount) {
                console.log("showProgress:", amount);
            },

            /**
             * Show location details to the user using the specified coordinates and product values at that point. When
             * coord is "", the location details should be cleared.
             *
             * @param coord a descriptive coordinates string, such as: "5.51° N, 24.79° E". Never null, but is ""
             *               when location indicator has been cleared/dismissed.
             * @param values a JSON array of JSON objects containing unit->value pairs. Example:
             *     {
             *         "wind": {
             *             "m/s" : "100° @ 7.3",
             *             "kn"  : "100° @ 14",
             ...
             *     }
             *
             *     ...where element 0 is for primary product and element 1 is for the overlay product. Never null. Can
             *     be empty array [] if no values at these coordinates.
             */
            showLocationDetails: function(coord, values) {
                console.log("showLocationDetails:", coord, values);
            }
        };
...
},{"./d3-geo-polyhedron":24,"./d3-geo-projection":25,"./earth":27,"./micro":30,"d3":3,"underscore":5}],30:[function(require,module,exports){
/**
...
!function() {
    ...
    µ.toUTCISO = function(date) {
        return date.getUTCFullYear() + "-" +
            µ.zeroPad(date.getUTCMonth() + 1, 2) + "-" +
            µ.zeroPad(date.getUTCDate(), 2) + " " +
            µ.zeroPad(date.getUTCHours(), 2) + ":00";
    };

    µ.toLocalISO = function(date) {
        return date.getFullYear() + "-" +
            µ.zeroPad(date.getMonth() + 1, 2) + "-" +
            µ.zeroPad(date.getDate(), 2) + " " +
            µ.zeroPad(date.getHours(), 2) + ":00";
    };
     ...
     */
    µ.ymdRedelimit = function(ymd, fromDelimiter, toDelimiter) {
        if (!fromDelimiter) {
     ...
     */
    µ.dateToUTCymd = function(date, delimiter) {
        return µ.ymdRedelimit(date.toISOString(), "-", delimiter || "");
    };
    ...
    /**
     * @param {Object?} out delegate logging to this object instead of the console.
     * @returns {Object} an object to perform logging, if/when the browser supports it.
     */
    µ.log = function(out) {
        var debug   = µ.bindup(out, "debug")   || µ.bindup(console, "debug")   || function() {};
        var info    = µ.bindup(out, "info")    || µ.bindup(console, "info")    || function() {};
        var error   = µ.bindup(out, "error")   || µ.bindup(console, "error")   || function() {};
        var time    = µ.bindup(out, "time")    || µ.bindup(console, "time")    || function() {};
        var timeEnd = µ.bindup(out, "timeEnd") || µ.bindup(console, "timeEnd") || function() {};
        function format(o) { return o && o.stack ? o + "\n" + o.stack : o; }
        return {
            debug:   function(s) { debug(format(s)); },
            info:    function(s) { info(format(s)); },
            error:   function(e) { error(format(e)); },
            time:    function(s) { time(format(s)); },
            timeEnd: function(s) { timeEnd(format(s)); }
        };
    };
    ...
    function xhrResolver(resource, d, error, result) {
        return error ?
            !error.status ?
                d.reject({status: -1, message: "Cannot load resource: " + resource + ": " + error, resource: resource}) :
     ...
     */
    µ.parse = function(hash, projectionNames, overlayTypes) {
        var option, result = {};
        //             1        2        3          4          5            6      7      8    9
        ...
        if (tokens) {
            var date = tokens[1] === "current" ?
                "current" :
                tokens[2] + "/" + µ.zeroPad(tokens[3], 2) + "/" + µ.zeroPad(tokens[4], 2);
            var hour = µ.isValue(tokens[5]) ? µ.zeroPad(tokens[5], 4) : "";
            result = {
    ...
    var Configuration = Backbone.Model.extend({
        ...
        toHash: function() {
            ...
            return [dir, attr.param, attr.surface, attr.level, anim, ol, proj, grid].filter(µ.isTruthy).join("/");
        },
    ...
    function lambertConformalConic(φ0, λ0, φ1, φ2, R) {
        φ0 = rad(φ0);
        φ1 = rad(φ1);
        φ2 = rad(φ2);
        λ0 = rad(λ0);
        var n = φ1 == φ2 ?
            Math.sin(φ1) :
            Math.log(Math.cos(φ1) / Math.cos(φ2)) / Math.log(Math.tan(π/4 + φ2/2) / Math.tan(π/4 + φ1/2));
        var sign = n < 0 ? -1 : 1;
        var F = Math.cos(φ1) * Math.pow(Math.tan(π/4 + φ1/2), n) / n;
        var p0 = R * F / Math.pow(Math.tan(π/4 + φ0/2), n);

        function projection(λ, φ) {
            var p = R * F / Math.pow(Math.tan(π/4 + rad(φ)/2), n);
            var theta = n * (µ.floorMod(π + rad(λ) - λ0, τ) - π); // (µ.floorMod(rad(λ) - λ0, τ) - π);
            return [
                p * Math.sin(theta),      // x
                p0 - p * Math.cos(theta)  // y
            ];
        }

        projection.invert = function(x, y) {
            var p = sign * Math.sqrt(x * x + Math.pow(p0 - y, 2));
            var theta = Math.atan(x / (p0 - y));
            return [
                deg(theta / n + λ0),                                  // λ
                deg(2 * Math.atan(Math.pow(R * F / p, 1 / n)) - π/2)  // φ
            ];
        };
...
},{"./decoder":26,"backbone":2,"d3":3,"underscore":5,"when":23}],31:[function(require,module,exports){
/**
...
!function() {
    ...
    var catalogs = {
        ...
        oscar: µ.loadJson([OSCAR_PATH, "oscar-catalog.json"].join("/"))
    };
    ...
    function buildProduct(overrides) {
        return _.extend({
            ...
            navigateTo: function(date) {
                return gfsDate(µ.dateToConfig(date));
            },
    ...
    function gfs1p0degPath(attr, type, surface, level) {
        ...
        var format = dir === "current" || new Date(attr.date) >= new Date("2014/11/24") ? ".epak" : ".json";
        var res = new Date(attr.date) < new Date("2015/03/11") || µ.isEmbeddedInIFrame() ? "1.0" : "0.5";
        var file = [stamp, type, surface, level, "gfs", res].filter(µ.isValue).join("-") + format;
    ...
    function wave30mPath(attr, type, surface, level) {
        var dir = attr.date, stamp = dir === "current" ? "current" : attr.hour;
        var format = ".epak";
        var file = [stamp, type, surface, level, "wave", "30m"].filter(µ.isValue).join("-") + format;
    ...
    function gfsDate(attr) {
        ...
        if (attr.date === "current") {
            ...
            hour = Math.floor(now.getUTCHours() / 3) * 3;
            return new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), hour));
        }
        var parts = attr.date.split("/");
        hour = Math.floor(+attr.hour.substr(0, 2) / 3) * 3;
        return new Date(Date.UTC(+parts[0], parts[1] - 1, +parts[2], hour));
    }
    ...
    var FACTORIES = {
        ...
        "wind": {
            matchesPrimary: _.matches({param: "wind"}),
        ...
        "misery_index": {
            ...
            create: function(attr) {
                return buildProduct({
                    ...
                    paths: [gfs1p0degPath(attr, "misery_index")],
                    date: gfsDate(attr),
                    builder: function(file) {
                        var isEpak = !!file.blocks;
                        var header = isEpak ? file.header : file;
            ...
            create: function(attr) {
                return buildProduct({
                    ...
                    builder: function(file) {
                        ...
                        var header = epak.header;
                        var vars = header.variables;
                        var cloud = vars.Cloud_water_entire_atmosphere;
            ...
            create: function(attr) {
                return buildProduct({
                    ...
                    builder: function(file) {
                        ...
                        var header = epak.header;
                        var vars = header.variables;
                        var pwat = vars.Precipitable_water_entire_atmosphere;
        ...
        "currents": {
            matchesPrimary: _.matches({param: "ocean", surface: "surface", level: "currents"}),
        ...
        "sea_surface_temp": {
            ...
            create: function(attr) {
                return buildProduct({
                    ...
                    paths: [rtg0p5degPath(attr, "sea_surface_temp")],
                    date: rtgDate(attr),
                    navigate: function(step) {
        ...
        "sea_surface_temp_anomaly": {
            ...
            create: function(attr) {
                return buildProduct({
                    ...
                    paths: [rtg0p5degPath(attr, "sea_surface_temp_anomaly")],
                    date: rtgDate(attr),
                    navigate: function(step) {
                        return rtgStep(this.date, step);
                    },
     ...
     * @returns {String}
     */
    function rtg0p5degPath(attr, type, surface, level) {
        var dir = attr.date === "current" ? "current" : attr.date.substr(0, 7);  // either "current" or "yyyy/mm"
        var stamp = dir === "current" ? "current" : µ.ymdRedelimit(attr.date, "/", "");  // either "current" or "yyyymmdd"
        var file = [stamp, type, surface, level, "rtg", "0.5"].filter(µ.isValue).join("-") + ".epak";
        return [OCEAN_PATH, dir, file].join("/");
    }

    function rtgDate(attr) {
        if (attr.date === "current") {
    ...
    function rtgStep(date, step) {
        var offset = (step > 1 ? 5 : step < -1 ? -5 : step), adjusted = new Date(date);
...
},{"./micro":30,"d3":3,"underscore":5,"when":23}],32:[function(require,module,exports){
