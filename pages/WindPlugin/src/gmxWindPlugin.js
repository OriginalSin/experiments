/*
 Leaflet.gmxWindPlugin, plugin for Gemixer wind layer.
*/
(function () {
/**
 ...
 */
VectorField.read = function(items, correctForSphere, data) {
	var field = [];
...
_this.allTime = (new Date().getTime() - _this.sendTime);
            console.log('Data loaded in worker', _this.allTime, e);
        };
        ...
        this.add = function (data) {
            ...
            var len = data.length,
                cnt = 0,
                items = new Float32Array(len * 4);
var field = VectorField.read(items, true, windData);
var mapProjection = new ScaledAlbers(1111, -75, this._canvas.height - 100, -126.5, 23.5);
...
*/
            for (var i = 0; i < len; i++) {
                var item = data[i].item,
                    ...
                    currentStyle = item.parsedStyleKeys,
                    isPoint = item.type === 'POINT' || item.type === 'MULTIPOINT';
                if (!style) {
                    style = {
                        icon: currentStyle.image,
                        width: currentStyle.sx * 2,
                        height: currentStyle.sx * 2
                    };
                ...
                items[cnt++] = prop[4];
                items[cnt++] = prop[5];
                items[cnt++] = currentStyle.iconColor;
/*
                if (gmx.styleHook) {
...
*/
    ...
    _getImageData: function (pt) {
        var canvas = L.DomUtil.create('canvas'),
            ctx = canvas.getContext('2d');
        canvas.width  = pt.width; canvas.height = pt.height;
        ctx.drawImage(pt.icon, 0, 0);
        var tt = ctx.getImageData(0, 0, pt.width, pt.height);
        return tt;
    },
